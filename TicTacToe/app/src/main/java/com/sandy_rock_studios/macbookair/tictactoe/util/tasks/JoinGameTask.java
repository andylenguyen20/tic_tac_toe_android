...
public class JoinGameTask extends AsyncTask<String, Void, Game> {
    private static final String QUERY_URL = "http://murmuring-escarpment-67851.herokuapp.com/games/join/";
    private String myID;
    private GameCreationHandler myHandler;
    public JoinGameTask(String userID, GameCreationHandler handler){
        myID = userID;
        myHandler = handler;
    }
    ...
    protected Game doInBackground(String... strings) {
        ...
        if(strings.length > 0){
            String opponentID = strings[0];
            urlString = urlString + "/" + opponentID;
        }
